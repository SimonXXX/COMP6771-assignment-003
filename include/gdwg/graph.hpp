...
namespace gdwg {
	...
	public:
		struct value_type { // edges
			// value_type(N const& src, N const& dst, E const& weight)
			// : from{src}
			// , to{dst}
			// , weight{weight} {}
			N from;
		...
		explicit graph<N, E>(N val) {
			node_list_.emplace(std::make_shared<node>(val));
		}
		...
		auto merge_replace_node(N const& old_data, N const& new_data) -> void {
			...
			// Throws: std::runtime_error("Cannot call gdwg::graph<N, E>::merge_replace_node on old or
			// new data if they don't exist in the graph") if either of is_node(old_data) or
		...
		// Postconditions: All iterators are invalidated.
		// Throws: std::runtime_error("Cannot call gdwg::graph<N, E>::erase_edge on src or dst if they
		// don't exist in the graph") if either is_node(src) or is_node(dst) is false.
		...
		// erased. If no such element exists, returns end(). Postconditions: All iterators are
		// invalidated. [Note: The postcondition is slightly stricter than a real-world container to
		// help make the assingment easier (i.e. we won’t be testing any iterators post-erasure). —end
		// note]
		...
		// postcondition is slightly stricter than a real-world container to help make the assingment
		// easier (i.e. we won’t be testing any iterators post-erasure). —end note]
		...
		[[nodiscard]] auto is_connected(N const& src, N const& dst) -> bool {
			...
			auto it1 = edge_list_.lower_bound(value_type{src, dst, E{}});
		...
		[[nodiscard]] auto weights(N const& from, N const& to) -> std::vector<E> {
			...
			auto w_it1 = edge_list_.lower_bound(value_type{from, to, E{-12121214}});
			if (w_it1 != edge_list_.end()) {
				// auto w_it2 = edge_list_.upper_bound(value_type{from, to, E{}});
				// auto temp = edge_list_.equal_range(value_type{from, to, E{}});
				while ((*w_it1).get()->get_to_node() == to) {
					weights_sequence.push_back((*w_it1).get()->get_edge_weight());
					++w_it1;
				}
			}
		...
		// stored edges.
		// Throws: std::runtime_error("Cannot call gdwg::graph<N, E>::weights if src or
		// dst node don't exist in the graph") if either of is_node(src) or is_node(dst) are false.
		...
		// end() if no such edge exists. Complexity: O(log (n) + log (e)), where n is the number of
		// stored nodes and e is the number of stored edges.
