...
TEST_CASE("2.3 Modifiers [gdwg.modifiers]") {
	SECTION("auto replace_node(N const& old_data, N const& new_data) -> bool") {
		...
		// Returns: false if a node that contains value new_data already exists and true
		// otherwise. Throws: std::runtime_error if the original data doesnt exist on any node
		auto g1 = gdwg::graph<int, std::string>{};
		g1.insert_node(5);
	...
	SECTION("auto merge_replace_node(N const& old_data, N const& new_data) -> void") {
		...
		// is_node(new_data) are false. [Note: Unlike Assignment 2, the exception message must
		// be used verbatim. ??end note] [Note: The following examples use the format
